...
import { destinations, tours, testimonials, stats } from "@/lib/data";
  ...
  return (
    <>
          ...
          {destinations.map((d) => (
            ...
            >
              <div className="aspect-[4/5] overflow-hidden rounded-md mb-4 bg-muted">
                <img src={d.image} alt={d.name} loading="lazy" className="size-full object-cover transition-transform duration-700 group-hover:scale-105" />
              </div>
              ...
              <Link key={t.slug} to="/tours/$slug" params={{ slug: t.slug }} className="group">
                <div className="relative aspect-[3/4] overflow-hidden rounded-md mb-5 bg-muted">
                  ...
                  <div className="absolute top-4 left-4 flex gap-2">
                    <span className="rounded-full bg-background/90 px-3 py-1 text-[10px] font-bold uppercase tracking-widest">{t.days} Days</span>
                  </div>
                </div>
      ...
      <section className="mx-auto max-w-7xl px-6 py-24 text-center">
        <h2 className="font-display text-3xl md:text-4xl mb-3">Join the journal.</h2>
        <p className="text-muted-foreground mb-8 max-w-md mx-auto">Seasonal trips, last-minute slots, and field stories from the North.</p>
        <form
          onSubmit={(e) => { e.preventDefault(); alert("Subscribed — we'll be in touch."); }}
          className="mx-auto max-w-md flex gap-2"
        >
          <input type="email" required placeholder="Email address" className="flex-1 rounded-full bg-muted px-5 py-3 text-sm outline-none focus:ring-2 focus:ring-accent" />
          <button className="rounded-full bg-foreground text-background px-6 py-3 text-xs font-bold uppercase tracking-widest">Subscribe</button>
        </form>
      </section>
    </>
  );
